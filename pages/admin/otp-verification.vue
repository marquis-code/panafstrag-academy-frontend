<template>
  <Transition name="fade">
    <section class="grid place-items-center lg:h-screen lg:items-center">
      <div class="w-full px-4 py-12 sm:px-6 sm:py-16 lg:w-1/2 lg:px-8 lg:py-24 mt-28">
        ...
        <form class="mx-auto mt-8 mb-0 max-w-md space-y-4" @submit.prevent="handleOtpVerification">
          <div>
            <label for="otp" class="sr-only">OTP</label>

            <div class="relative">
              <input v-model.trim="form.otp" type="tel"
          ...
          <div class="flex items-center justify-between flex-col space-y-5">
            <button :disabled="loading" type="submit" :class="[loading ? 'cursor-not-allowed opacity-20' : '']"
...
export default {
  ...
  methods: {
    ...
    async handleOtpVerification() {
      ...
      const payload = {
        otp: this.form.otp,
        userId: this.$store?.state?.app?.auth.userId
      }
      await this.$axios
        .post('https://academy-api-service.onrender.com/api/auth/verifyOtp', payload)
        .then((response) => {
          console.log(response.data)
          // this.$store.commit('app/resetState')
          this.$store.commit('app/setAuthUser', response?.data)
